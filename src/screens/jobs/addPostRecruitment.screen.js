...
import {GET_ADD_POST_RECRUTMENT, GET_LIST_TYPE_JOB} from '../../actions/postRecrument.action';
...
export class AddPostRecruitmentScreen extends Component {
    constructor(props){
      super(props);
      this.state = {
        title : "",
   ...
   getListTypeJob = () => {
    ...
    render() {
      const {title, nameTypeJob, salary, degree, dateWord, deadline, specialize, status, description, listTypesJobs} = this.state;
        return (
            <View style={{flex:1,  padding:10}}>
              <KeyboardAvoidingView
                ...
                <ScrollView showsVerticalScrollIndicator={false} 
                  ...
                  <View style={styles.imputContainer}>
                  <Text>Chọn loại ngành: </Text>
                    <View style={styles.subInputContainer}>
                      <RNPickerSelect
                          items={listTypesJobs}
                          onValueChange ={item => this.setState({
                              nameTypeJob: item.value
                          })}
                      />
                    </View>
                  </View>
            ...
            <View style={{height:42}}>
              <TouchableOpacity
                  ...
                  activeOpacity={0.6}
                  onPress={() => {this.addPostRecruitment()}}
                  >
    ...
    addPostRecruitment = () => {
      ...
      this.props.addPostRecruitment(data);
      Toast.show("Tạo bài đăng thành công!");
...
const mapDispatchToProps = dispatch => {
  return {
    addPostRecruitment : (data) => dispatch({type: GET_ADD_POST_RECRUTMENT, data}),
    getRefresh : () => dispatch({type: GET_REFRESH}),
    getTypeJobs : () => dispatch({type: GET_LIST_TYPE_JOB})
  }
}
export default connect(mapStateToProps, mapDispatchToProps) (AddPostRecruitmentScreen)
